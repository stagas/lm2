...
import { workletImports } from './worklet-imports.ts'
...
export class DspProcessor extends AudioWorkletProcessor {
  ...
  async invalidateRecordings(program$: number) {
    if (!this.core) return
  ...
  process(
    ...
    try {
      ...
      if (!isRestartWithProgram && control !== this.lastControl) {
        ...
        else if (control === ControlOp.Pause
          && (this.state === 'running' || this.state === 'fade-in' || this.state === 'preparing'))
        {
          this.state = 'fade-out'
          this.shouldReset = false
        }
