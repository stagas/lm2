...
import type { ExpanderRef } from './types.ts'
...
export function createExpanderVisitor(src: string, refs: ExpanderRef[]) {
  ...
  return {
    visitCall(expr: any): void {
      ...
      if (calleeName === 'expander') {
        ...
        const knobParams: ExpanderRef['knobParams'] = []
        ...
        refs.push({
          expanderIndex: getIndexFromCall(expr),
          loc: calleeLoc,
          aboveLoc,
          callLoc: expr.loc,
          ...
          params: {
            ...
            ratio: getNumberOrDefault(ratioExpr, 0.5),
