...
function getPosArgs(call: any): any[] {
  return (call.args ?? []).filter((a: any) => a?.kind === 'pos')
}

function findNamedArg(call: any, name: string): any | null {
...
function getNumberOrDefault(expr: any, fallback: number): number {
...
function buildLineStarts(src: string): Int32Array {
  const starts: number[] = [0]
  for (let i = 0; i < src.length; i++) if (src[i] === '\n') starts.push(i + 1)
  return Int32Array.from(starts)
}

function indexFromLoc(src: string, lineStarts: Int32Array, loc: Loc): number {
  const line = Math.max(1, loc.line | 0)
  const col = Math.max(1, loc.column | 0)
  const base = lineStarts[line - 1] ?? 0
  return Math.max(0, Math.min(src.length, base + (col - 1)))
}
...
function computeAboveLoc(src: string, lineStarts: Int32Array, calleeLoc: Loc): Loc {
