...
let currentCompiledMini: ReturnType<typeof compileMiniNotation> | undefined
...
async function createProgram() {
  ...
  currentCompiledMini = compileMiniNotation(currentSequenceString)
  data.arrays[1].raw.fill(0)
  data.arrays[1].raw[2] = SEQ_HISTORY_SIZE
  data.arrays[1].raw.set(currentCompiledMini.bytecode, ARRAY_HEADER_SIZE)
...
function updateSequence(newSequence: string) {
  try {
    ...
    if (program) {
      program.data.arrays[1].raw.fill(0)
      program.data.arrays[1].raw[2] = SEQ_HISTORY_SIZE
      program.data.arrays[1].raw.set(currentCompiledMini.bytecode, ARRAY_HEADER_SIZE)
    }
