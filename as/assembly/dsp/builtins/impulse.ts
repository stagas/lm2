...
export function callImpulse(
  ...
  const impulse = program.gensPool.get(Op.Impulse) as Impulse
  impulse.hz$ = hz$
  impulse.trig$ = trig$
  impulse.offset$ = offset$
  impulse.process(out$, length)
