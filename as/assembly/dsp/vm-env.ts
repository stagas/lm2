...
export class VmEnv {
  ...
  load(sym: i32, stack: VmStack, audio: VmAudio, program: Program, length: i32): void {
    ...
    if (sym === VmSym.Fdn) {
      stack.push(VmTag.Builtin, 0.0, VmSym.Fdn)
      return
    }
    if (sym === VmSym.Velvet) {
      stack.push(VmTag.Builtin, 0.0, VmSym.Velvet)
      return
    }
    if (sym === VmSym.Dc) {
      stack.push(VmTag.Builtin, 0.0, VmSym.Dc)
      return
    }
    if (sym === VmSym.Mini) {
    ...
    if (sym === VmSym.Slp) {
      stack.push(VmTag.Builtin, 0.0, VmSym.Slp)
      return
    }
    if (sym === VmSym.Shp) {
      stack.push(VmTag.Builtin, 0.0, VmSym.Shp)
      return
    }
    if (sym === VmSym.Sbp) {
      stack.push(VmTag.Builtin, 0.0, VmSym.Sbp)
      return
    }
    if (sym === VmSym.Sbs) {
      stack.push(VmTag.Builtin, 0.0, VmSym.Sbs)
      return
    }
    if (sym === VmSym.Speak) {
      stack.push(VmTag.Builtin, 0.0, VmSym.Speak)
      return
    }
    if (sym === VmSym.Sap) {
    ...
    if (sym === VmSym.Oversample) {
      stack.push(VmTag.Builtin, 0.0, VmSym.Oversample)
      return
    }
    if (sym === VmSym.Glide) {
